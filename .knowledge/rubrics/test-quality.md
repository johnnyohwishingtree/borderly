...
Evaluate test files (following `.knowledge/templates/test.md`) against these criteria.
...
- No snapshot files — use inline assertions or explicit checks

## Clarity (weight: 20%)
- Test names describe behavior, not implementation
- Factory functions abstract repeated test data
- One logical assertion group per `it()` block
- Component tests use RNTL accessibility queries (getByRole > getByLabelText > getByTestId)
