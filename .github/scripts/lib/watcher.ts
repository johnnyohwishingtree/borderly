...
export function getPRCIConclusion(pr: number, repo: string): string {
  // Check required CI checks: "test" (unit) and "test-chromium" (E2E rollup).
  // test-chromium is a rollup job that gates on all matrix shards.
  ...
  try {
    ...
    // Both required checks must be present and succeeded.
    const hasTest = checks.some(c => c.name === 'test');
    const hasChromium = checks.some(c => c.name === 'test-chromium');
...
export async function checkPR(
  ...
  if (mergeable === 'CONFLICTING') {
    // Auto-generated PRs (screenshots, chores) with conflicts should be closed
    // since they'll be regenerated on the next merge to master.
    const autoGeneratedPrefixes = ['chore/update-screenshots'];
    const isAutoGenerated = autoGeneratedPrefixes.some(p => pr.branch.startsWith(p));
    if (isAutoGenerated) {
      execOrDefault('gh', ['pr', 'close', String(pr.number), '--repo', repo,
        '--comment', 'Closing — auto-generated PR has conflicts and will be regenerated on next merge.'], '');
      return { action: 'close-stale', detail: `Auto-generated PR with conflicts — closed (branch: ${pr.branch})` };
    }

    // For real PRs, dispatch conflict resolution
  ...
  // Missing CI — use update-branch to trigger pull_request synchronize event.
  // workflow_dispatch runs don't attach to PR check status, so dispatching
  // test.yml/e2e-smoke.yml leaves the PR stuck with "Waiting for status".
  if (!ciConclusion || ciConclusion === 'null') {
    if (commitAgo >= graceMinutes) {
      try {
        await github.updateBranch(pr.number);
        return { action: 'retrigger-ci', detail: `No CI check, ${commitAgo}m stale — triggered branch update (synchronize event)` };
      } catch {
        // updateBranch fails if already up-to-date — fall back to dispatch
        await github.dispatchWorkflow('test.yml', pr.branch);
        await github.dispatchWorkflow('e2e-smoke.yml', pr.branch);
        return { action: 'retrigger-ci', detail: `No CI check, ${commitAgo}m stale — branch up-to-date, dispatched test + e2e-smoke` };
      }
    }
...
/** @deprecated Never close PRs automatically — use branch cleanup instead */
export function closeOrphanPR(_pr: number, _branch: string, _repo: string): void {
  // Disabled — closing PRs destroys work (PR #505 was killed by this).
}
