/**
 * Tests for Japan (JPN) field mapping config.
 *
 * Validates:
...
// Collect all field IDs from the JPN schema sections
const schemaFieldIds = new Set<string>(
  (JPN_SCHEMA.sections as SchemaSection[]).flatMap((section) =>
...
describe('JPN field mapping', () => {
  it('has a non-empty fieldMappings object', () => {
    expect(Object.keys(JPN_MAPPING.fieldMappings).length).toBeGreaterThan(0);
  });
  ...
  it('has a non-empty portalUrl', () => {
    expect(JPN_MAPPING.portalUrl.length).toBeGreaterThan(0);
  });
  ...
  describe('each field mapping', () => {
    Object.entries(JPN_MAPPING.fieldMappings).forEach(([key, mapping]) => {
      describe(`field: ${key}`, () => {
        ...
        it('has a fieldId that exists in the JPN schema', () => {
  ...
  it('has core required fields: surname, givenNames, passportNumber, nationality, dateOfBirth', () => {
    const fieldIds = Object.keys(JPN_MAPPING.fieldMappings);
