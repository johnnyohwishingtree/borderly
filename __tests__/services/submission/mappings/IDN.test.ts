/**
 * Tests for Indonesia (IDN) field mappings.
 *
 * Validates that:
 * - All field IDs in the mapping exist in the IDN JSON schema
...
function extractSchemaFieldIds(schema: typeof IDNSchema): Set<string> {
...
describe('IDN field mappings', () => {
  const schemaFieldIds = extractSchemaFieldIds(IDNSchema);
  ...
  it('country code is IDN', () => {
    expect(IDN_MAPPING.countryCode).toBe('IDN');
  });

  it('has at least one automation step', () => {
    expect(IDN_MAPPING.steps.length).toBeGreaterThan(0);
  });
  ...
  describe('each field mapping', () => {
    ...
    it.each(fieldIds)('field "%s" exists in the IDN JSON schema', (fieldId) => {
