...
test.skip('TripListScreen no longer exists', () => {
  expect(existsSync(
    resolve(ROOT, 'src/screens/trips/TripListScreen'),
  )).toBe(false);
});

test.skip('TripDetailScreen no longer exists', () => {
  expect(existsSync(
    resolve(ROOT, 'src/screens/trips/TripDetailScreen'),
  )).toBe(false);
});

test.skip('CreateTripScreen no longer exists', () => {
  expect(existsSync(
    resolve(ROOT, 'src/screens/trips/CreateTripScreen'),
  )).toBe(false);
});

test.skip('no duplicate trip functionality', () => {
  // DuplicateTripModal, tripDuplicateService — removed
});

test.skip('no trip naming in any screen', () => {
  // No "Trip Name" input field anywhere in the app
});

test.skip('forms tab is the first tab', () => {
