...
import { ValidationConfig, DEFAULT_CONFIG } from './submissionValidatorTypes';
import { validateCountrySpecific, extractFormData } from './countryValidators';
...
export class SubmissionValidator {
  ...
  async validateSubmission(
    ...
    const countryValidation = validateCountrySpecific(countryCode, filledForm);
   ...
   */
  validateSessionData(sessionData: Record<string, unknown>): SecurityValidationResult {
    const result: SecurityValidationResult = {
      isValid: true,
      warnings: [],
      errors: [],
  ...
  private validateDataSize(filledForm: FilledForm): SecurityValidationResult {
    ...
    const dataSize = JSON.stringify(filledForm).length;
...
// Re-export extractFormData for consumers that need it
