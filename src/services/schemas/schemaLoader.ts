...
const CountryFormSchemaValidator = z.object({
  ...
  portalName: z.string(),
  submissionDeadlineHours: z.number().nonnegative(),
  recommendedLeadTimeHours: z.number().positive(),
  submissionWindowNote: z.string(),
  submission: z.object({
...
function convertToStrictSchema(parsed: any): CountryFormSchema {
  ...
  return {
    ...
    submissionDeadlineHours: parsed.submissionDeadlineHours,
    recommendedLeadTimeHours: parsed.recommendedLeadTimeHours,
    submissionWindowNote: parsed.submissionWindowNote,
