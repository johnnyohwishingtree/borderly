...
/**
 * Checks if an error is actually storage/keychain related
 */
 ...
 * In __DEV__ mode, surfaces the raw error so root causes are never hidden.
 */
export function logError(
  ...
  // In dev mode, also log the raw error prominently so it's never hidden
  // behind wrapped/classified error messages
  if (__DEV__) {
