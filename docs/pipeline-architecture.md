...
- **Solution**: Replaced per-command Bash whitelist with `Bash(*)` across all workflows (verify-merge, pipeline-doctor, review-fix, claude.yml). Added `permissions.allow` with `Edit(*)`, `Write(*)`, `MultiEdit(*)` to pre-approve file writes (Claude Code's internal permission system blocks writes to sensitive paths like `.github/workflows/` even when `allowedTools` includes `Edit`/`Write`). Also reduced `maxTurns` to 15 for fix jobs, added `show_full_output: true` for debugging, simplified fix prompt, and added a post-step that fails the job if >5 permission denials occur (so it shows red and aborts the retry loop instead of silently wasting turns).
