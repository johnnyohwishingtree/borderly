...
- **Solution**: All three auto-approve paths in `review-guardian.yml` (`request-approval`, `ensure-review`, `auto-approve-after-claude`) now fetch inline PR review comments via `gh api repos/.../pulls/N/comments` and match against `![high]` and `![critical]` badge patterns (regex: `!\[(critical|high)\]`) in addition to the existing keyword patterns. If any are found AND the review threads are still unresolved, auto-approve is skipped and a comment is posted explaining why. If all threads are resolved (meaning the feedback was addressed by review-fix), the badge check is bypassed and the PR is approved. `review-relay.yml` remains the single owner of triggering Claude to fix the flagged issues.
