...
/**
 * UK Electronic Travel Authorisation Submission E2E Tests
 *
 * Verifies the form generation and submission guide for the UK
        ...
        'e2e-trip-gbr': [
          {
            ...
            arrivalDateISO: '2026-06-15',
            departureDateISO: '2026-06-25',
            flightNumber: 'BA178',
            airlineCode: 'BA',
...
test.describe('UK ETA Submission', () => {
  let jsErrors: string[];
  ...
  test('trip detail shows UK leg card', async ({ page }) => {
    ...
    // Open the trip
    const tripCard = page.getByTestId('trip-card-UK Trip');
    const tripCardCount = await tripCard.count();
    if (tripCardCount > 0) {
      await tripCard.click();
      // Should show the itinerary with GBR leg
      await expect(page.getByText('Itinerary', { exact: true })).toBeVisible({ timeout: 10000 });
      await expect(page.getByTestId('leg-card-GBR')).toBeVisible({ timeout: 5000 });
    }
  });

  test('UK leg form renders with DynamicForm', async ({ page }) => {
    ...
    await tripCard.click();
    await expect(page.getByTestId('leg-card-GBR')).toBeVisible({ timeout: 10000 });
    await page.getByTestId('leg-card-GBR').click();
