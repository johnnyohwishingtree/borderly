// Web mock for lucide-react-native — renders inline SVGs so icons are visible
const React = require('react');

// SVG path data for icons used in the app (from lucide icon set)
const ICON_PATHS = {
  ...
  PlaneTakeoff: '<path d="M2 22h20"/><path d="M6.36 17.4 4 17l-2-4 1.1-.55a2 2 0 0 1 1.8 0l.17.1a2 2 0 0 0 1.8 0L8 12 5 6l.9-.45a2 2 0 0 1 2.09.2l4.02 3a2 2 0 0 0 2.1.2L22 5l-1.97 10.13a2 2 0 0 1-1.66 1.6L12 18l-4.33-.86a2 2 0 0 1-.64-.26z"/>',
...
function createIcon(name) {
  const Icon = function(props) {
    ...
    const pathData = ICON_PATHS[name] || '<circle cx="12" cy="12" r="10"/>';
    return React.createElement('svg', {
      ...
      style: style,
      'data-icon-name': name,
    }, React.createElement('g', { dangerouslySetInnerHTML: { __html: pathData } }));
  };
  Icon.displayName = name;
  return Icon;
}

// Use a Proxy so any icon import resolves without listing every name
module.exports = new Proxy({}, {
  get: (_target, name) => {
    if (name === '__esModule') return true;
    if (name === 'default') return new Proxy({}, { get: (_t, n) => createIcon(n) });
